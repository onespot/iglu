 ...
 */
package com.snowplowanalytics.iglu.schemaddl.spectrum.generators

// Scalaz
import com.snowplowanalytics.iglu.schemaddl.redshift.{Column => _, CreateTable => _, _}
import com.snowplowanalytics.iglu.schemaddl.spectrum._
import com.snowplowanalytics.iglu.schemaddl.{FlatSchema, PropertyList, spectrum}
...
// This project
import com.snowplowanalytics.iglu.schemaddl.redshift.RefTable
import com.snowplowanalytics.iglu.schemaddl.redshift.generators.EncodeSuggestions._
import com.snowplowanalytics.iglu.schemaddl.spectrum.generators.TypeSuggestions._
...
object DdlGenerator {
  ...
  : CreateTable = {
    ...
    else getAtomicTableDdl(dbSchema, name, columns, location)
  }

  // Columns with data taken from self-describing schema
  private[spectrum] val selfDescSchemaColumns = List(
  ...
  private[spectrum] val parentageColumns = List(
    Column("root_id", RedshiftChar(36), Set(CompressionEncoding(RawEncoding)), Set(Nullability(NotNull))),
    Column("root_tstamp", RedshiftSpectrumTimestamp, Set(CompressionEncoding(LzoEncoding)), Set(Nullability(NotNull))),
    Column("ref_root", RedshiftVarchar(255), Set(CompressionEncoding(RunLengthEncoding)), Set(Nullability(NotNull))),
    // Spectrum doesnt support struct datatypes so we exclude this
  ...
  /**
    ...
    */
  private def getAtomicTableDdl(dbSchema: Option[String], name: String, columns: List[Column], location: String): CreateTable = {
    val schema = dbSchema.getOrElse("spectrum")
  ...
  /**
    ...
    * @return full CREATE TABLE statement ready to be rendered
    */
  private def getRawTableDdl(dbSchema: Option[String], name: String, columns: List[spectrum.Column]): CreateTable = {
