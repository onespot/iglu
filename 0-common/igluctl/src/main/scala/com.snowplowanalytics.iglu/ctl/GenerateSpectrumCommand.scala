...
import com.snowplowanalytics.iglu.schemaddl.spectrum.generators.JsonPathGenerator
import com.snowplowanalytics.iglu.schemaddl.redshift.{CommentBlock, CommentOn, CreateSchema, Empty}
...
import com.snowplowanalytics.iglu.schemaddl.{FlatSchema, _}
import com.snowplowanalytics.iglu.schemaddl.redshift.generators.{DdlFile}
import com.snowplowanalytics.iglu.schemaddl.spectrum._
import com.snowplowanalytics.iglu.schemaddl.spectrum.generators.DdlGenerator
...
// This library
import com.snowplowanalytics.iglu.ctl.FileUtils._
import com.snowplowanalytics.iglu.ctl.Utils._

case class GenerateSpectrumCommand(
                                    input: File,
                                    location: String,
                                    output: File,
                                    db: String = "spectrum", // Isn't checked anywhere
                                    withJsonPaths: Boolean = false,
                                    rawMode: Boolean = false,
                                    dbSchema: Option[String] = None, // empty for raw, "spectrum" for non-raw
                                    varcharSize: Int = 4096,
                                    splitProduct: Boolean = false,
                                    noHeader: Boolean = false,
                                    force: Boolean = false) extends Command.CtlCommand {

  import GenerateSpectrumCommand._
    ...
    jsons match {
      ...
      case someJsons =>
        val outputs =
          ...
          else transformSelfDescribing(someJsons,location)
  ...
  /**
    ...
    */
  private[ctl] def transformSelfDescribing(files: List[JsonFile], location: String): DdlOutput = {
    val dbSchemaStr = dbSchema.getOrElse("spectrum")
    ...
    val validatedDdls = schemas.map(schema => selfDescSchemaToDdl(schema, dbSchemaStr,location).map(ddl => (schema.self, ddl)))
  ...
  /**
    ...
    */
  private[ctl] def selfDescSchemaToDdl(schema: IgluSchema, dbSchema: String, location: String): Validation[String, TableDefinition] = {
    val ddl = for {
      flatSchema <- FlatSchema.flattenJsonSchema(schema.schema, splitProduct)
    } yield produceTable(flatSchema, schema.self, dbSchema, location)
    ddl match {
  ...
  /**
    ...
    */
  private def produceTable(flatSchema: FlatSchema, schemaKey: SchemaKey, dbSchema: String, location: String): TableDefinition = {
    val (path, filename) = getFileName(schemaKey)
    val tableName = StringUtils.getTableName(schemaKey)
    val dropTable = DropTable(dbSchema+"."+tableName)

    val schemaCreate = CreateSchema(dbSchema)
    val table = DdlGenerator.generateTableDdl(flatSchema, tableName, Some(dbSchema), varcharSize,location, rawMode)
    val ddlFile = DdlFile(header ++ List(Empty, table, Empty))
  ...
  private def produceRawTable(flatSchema: FlatSchema, fileName: String): TableDefinition = {
    ...
    val table = DdlGenerator.generateTableDdl(flatSchema, name, dbSchema, varcharSize, location, rawMode)
...
object GenerateSpectrumCommand {
